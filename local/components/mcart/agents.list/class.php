...
class AgentsList extends CBitrixComponent implements Controllerable, Errorable
{
    ...
    final public function onPrepareComponentParams($arParams): array
    {
        ...
        if (is_null($arParams["NEWS_COUNT"])) {
    ...
    final public function executeComponent(): void
    {
        ...
        $userOption = CUserOptions::GetOption($category, $name);

        if ($userOption) {
            $this->arResult['STAR_AGENTS'] = $userOption;
        } else {
    ...
    private function getFieldListValue(array $arHlblock, string $fieldName): array
    {
        ...
        if ($fieldID) {
             ...
             */
            $rsEnum = CUserFieldEnum::GetList(array(), array("USER_FIELD_ID" => $fieldID));
    ...
    private function getAgents(string $entity, array $arTypeAgents): array
    {
        ...
        while ($arAgent = $rsAgents->fetch()) {
             ...
             */
            $arAgent["UF_TYPE_OF_ACTIVITY_VALUE"] = $arTypeAgents[$arAgent["UF_TYPE_OF_ACTIVITY"]]["VALUE"];
    ...
    public function clickStarAction($agentID)
    {
         ...
         * 4. Записать $value (результат) в бд, метод CUserOptions::SetOption
        ...
        CUserOptions::SetOption($category, $name, $userOption);
        
        $result["ID OF SELECTED AGENTS"] = $userOption;
        return $result;
    }
}
